...
#XXX IMPORTANT  This version of the script assumes that you are trying to work with a set of files created by splitting a large download into pieces (so that they can be processed with reasonable RAM). Typically, downloaded are of the form 'myRunName_R1.fastq' and a program like fastq-splitter.pl will create files with names like 'myRunName_R1.part-01.fastq'. 

#First, what is the filename before _R1 or _R2 and the rest? (Usually the same, sometimes not...)
my $baseNameR1 = 'test3412_New_Pink';
my $baseNameR2 = 'test3412_New_Pink';

#How many R1/R2 file sets are there?
my $setNum = '10';

#Here we are going to assume that these parts are not compressed:
my $ext = 'NONE';

#What is the full location name of the run folder that will be processed? Starting with /home (no trailing /)
my $run = '/home/kdlugosch/data_temp/Heather_run_2017_04_21';
...
my $barcodeKey = '/home/kdlugosch/data_temp/Heather_run_2017_04_21/Barcodes_2017_04_21_Illumina_150bpPE_vKMD';

#What is the full location where you want the resulting folder of UNCOMPRESSED FASTQ files for each individual?
my $outLocation = '/home/kdlugosch/data_temp/Heather_run_2017_04_21';
#A new folder will be made in that location. What name do you want for this?
my $outDir = "$outLocation/split";
...
unless (-e "$run/$baseNameR1\_R1.part-01.fastq") { die "\n\nError: file $run/$baseNameR1\_R1.part-01.fastq does not exist!\n\n"; }
unless (-e "$run/$baseNameR2\_R2.part-01.fastq") { die "\n\nError: file $run/$baseNameR2\_R2.part-01.fastq does not exist!\n\n"; }
...
for (my $j = 1; $j <= $setNum; $j++) {
	...
	if ($j < 10) { 
		$R1INFILE = "$baseNameR1\_R1.part-0$j.fastq"; #single digit file count needs an extra zero in the name
		$R2INFILE = "$baseNameR2\_R2.part-0$j.fastq"; #single digit file count needs an extra zero in the name
	}
	else {
		$R1INFILE = "$baseNameR1\_R1.part-$j.fastq"; #double digit file count needs one less zero in the name
		$R2INFILE = "$baseNameR2\_R2.part-$j.fastq"; #double digit file count needs one less zero in the name
	}
